...
const char *in = "long_frozen_input.webm";
const char *NOISE_THRESH  = "-60dB";
...
static int init_filters(void)
{
    ...
    // SNPRINTF FOR FREEZEDETECT FILTER DESCRIPTION
    char filter_descr[128];
    snprintf(filter_descr, sizeof(filter_descr), "freezedetect=noise=%s:duration=%g", NOISE_THRESH, MIN_DURATION);
...
static void check_for_freeze(AVFrame *frame)
{
    AVDictionaryEntry *e = NULL;
    e = av_dict_get(frame->metadata, "lavfi.freezedetect.freeze_start", NULL, 0);
    if(e){
        printf("\nFrozen frame detected. Freeze time starts at %.2f seconds\n", atof(e->value));
    }
    e = av_dict_get(frame->metadata, "lavfi.freezedetect.freeze_end", NULL, 0);
    if(e){
        printf("\nFreeze time over at %.2f seconds\n", atof(e->value));
    } 
    AVDictionaryEntry *dur = av_dict_get(frame->metadata, "lavfi.freezedetect.freeze_duration", NULL, 0);
    if(dur){
        printf("\nFrames were frozen for : %.2f seconds\n", atof(dur->value));
    }
}
int main(void)
{
