...
const char *out = "output_h264_NOSCC.mp4";
...
int main() {
    ...
    for (int s = 0; s < i->nb_streams; s++) {
        ...
        if (is->codecpar->codec_type == AVMEDIA_TYPE_VIDEO && video_index == -1) {
            ...
            // Fixed 500 kbps same budget as the AV1 program
    ...
    while (av_read_frame(i, p) >= 0) {
        if (p->stream_index != video_index) { av_packet_unref(p); continue; }
    ...
    if (!(o->oformat->flags & AVFMT_NOFILE)) avio_closep(&o->pb);
